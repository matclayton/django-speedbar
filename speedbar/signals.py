...
def store_request_trace(sender, **kwargs):
    ...
    # Calculate values before doing any cache writes, so the cache writes
    ...
    if request_trace.persist_details:
        ...
        all_details = dict(
            details for details in details_tuples if details[1] is not None)
    if request_trace.persist_log:
        speedtracer_log = request_trace.stacktracer.speedtracer_log()

    if request_trace.persist_details:
        cache.set(
            DETAILS_PREFIX + request_trace.id, all_details, DETAILS_CACHE_TIME)
    if request_trace.persist_log:
        cache.set(TRACE_PREFIX + request_trace.id,
                  speedtracer_log, DETAILS_CACHE_TIME)
